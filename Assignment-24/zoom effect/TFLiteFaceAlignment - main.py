...
def zoomEffect(frame, landmarks):
    ...
    frame_target = frame[int(y-(h/2)):int(y+h+(h/2)), int(x-(w/2)):int(x+w+(w/2))]
    ...
    frame[int(y-(h/2)):int(y+h+(h/2)), int(x-(w/2)):int(x+w+(w/2))] = res * 255
    return frame
...
if __name__ == '__main__':
    ...
    file_path = 'video save/output.mp4'
    frame_width, frame_height = int(video_cap.get(3)), int(video_cap.get(4))
    size = (frame_width, frame_height)
    frame_rate = 10
    fourcc = cv2.VideoWriter_fourcc(*'MP4V')
    
    video_writer = cv2.VideoWriter(file_path, fourcc, frame_rate, size)
    while True:
        ...
        for pred in fa.get_landmarks(frame, boxes):
            ...
            #     print(p, index)
            #     cv2.circle(frame, tuple(p), 1, (125, 255, 125), 1, cv2.LINE_AA)
            #     cv2.putText(frame, str(index), tuple(p), cv2.FONT_HERSHEY_SIMPLEX, 0.25, (0, 0, 255), 1)
            
            frame = zoomEffect(frame, landmarks_left_eye)
            frame = zoomEffect(frame, landmarks_right_eye)
            frame = zoomEffect(frame, landmarks_lips)
